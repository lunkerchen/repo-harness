...
`Stop.default` routes through `stop-orchestrator.sh`. On Codex, dispatcher stdout stays quiet for ordinary successful hooks and also suppresses Stop decision JSON, because current Codex Desktop rejects that turn-finalization stdout as an unsupported content type. The hook still refreshes handoff state and records one-shot planning/delegation guard state; success stderr such as handoff refresh noise remains suppressed. Claude keeps the direct `stop-orchestrator.sh` decision JSON path.
