...
8. Tell the user that ChatGPT Connector setup still requires an HTTPS tunnel or equivalent public HTTPS `/mcp` endpoint, then manual connector creation in ChatGPT settings using the recorded server name. For recurring use, prefer a stable hostname from a named tunnel or reserved domain and run `repo-harness mcp setup chatgpt --endpoint <https-url>/mcp --server-name <name>`; this stores the endpoint and server name in ignored local config while tracked guides stay placeholder-only. Account-less quick tunnels are only for smoke tests because their URL changes. Keep OAuth passphrases and bearer tokens redacted.
   - For GPT Pro read-back, tell the user to start a fresh ChatGPT Web chat, enable `Deep research`, then select the recorded Connector chip in the composer. Pro Extended alone or prompt text that names the Connector is not enough to prove the MCP-capable surface is active.
   - In user-scope full-disk mode, tell GPT Pro to resolve repo-like user input such as `my-app/` by calling `discover_harness_repos` with `query` before falling back to exact absolute paths. The MCP tools can resolve `repo_path` aliases after full-disk authorization; GPT should not require the user to know absolute local paths.
   - If the user wants a hosted GitHub repo review instead of local MCP read-back, tell them to keep `Deep research` enabled, select GitHub in the same app picker, and choose the exact `<owner>/<repo>` entry. Report this as GitHub repo evidence, not MCP evidence.
...
- If Pro can see the Connector in settings but the chat does not expose MCP, retry once in a fresh chat with `Deep research` enabled and the recorded Connector chip selected before classifying it as `surface_blocked`.
- If Pro can read the GitHub repository but not the local MCP sidecar, classify the run as GitHub-backed review only; it cannot validate unpushed local changes or ignored repo-harness workflow state.
