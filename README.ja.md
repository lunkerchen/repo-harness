...
- **明確なコマンド境界。** `repo-harness update` は user-level CLI/runtime の更新だけを担当し、`repo-harness adopt` が repo-local workflow の install、refresh、migration を担当します。
- **Package-dispatched helper runtime。** 生成された `scripts/*` wrapper は `repo-harness run <helper>` 経由で installed package に委譲でき、adopted repo がすべての helper implementation を vendor する必要はありません。
- **8 つの managed hook routes。** Claude/Codex adapters が install する route matrix を README に明記しました：session context、edit guards、delegated-agent routing、post-edit/post-bash observers、always-on trace、prompt routing、stop closeout。
- **Release-ready install examples。** First 5 Minutes は machine bootstrap、user-level updates、read-only setup audit、repo-local adoption を分けて示します。
