...
- Follow-up correction: Oracle doctor readiness must require every flag repo-harness may send at runtime (`--browser-archive`, `--browser-follow-up`, `--followup`, etc.), not only the initial consult flags. Explicit binary configuration (`--oracle-bin` or `REPO_HARNESS_ORACLE_BIN`) must fail closed when invalid and must not silently fall through to PATH.
