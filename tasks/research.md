...
- `.ai/hooks/` remains the shared source of truth; `.claude/settings.json` and `.codex/hooks.json` are adapter surfaces and repo-local `.claude/hooks/` should not be generated by default.
...
- Claude uses hook-triggered automation through `.claude/settings.json`; Codex uses the same hook commands through `.codex/hooks.json`.
